...
class TargetView: UIView {
    ...
    func show(){
        ...
        func hide(){
            let finalFrame = CGRect(origin: CGPoint(x: bounds.width / 2, y: finalFrame.height / 2), size: CGSize.zero)
            isHidden = false
    ...
    override func draw(_ rect: CGRect) {
        ...
        let frame = CGRect(
            origin: CGPoint(x: center.x - size / 2, y: center.y - size / 2), size: CGSize(width: size, height: size))
